...
namespace ManagedWinapi.Windows.Contents {
    internal class ListViewParser : WindowContentParser {
        ...
        internal override WindowContent ParseContent(SystemWindow? sw) {
            ...
            if (o.RoleIndex != 33) return new ListContent("EmptyListView", -1, null, Array.Empty<string>());
                        ...
                        try {
                            ...
                            if (cols == null && values.Count == 0) { hdr = null; } else {
                                ...
                                foreach (string header in hdr) {
                                    string h = "; " + header + ": ";
                                    if (!tmpCols.Contains(h)) continue;
                                    usedHdr.Add(header);
                                    tmpCols = tmpCols[(tmpCols.IndexOf(h, StringComparison.Ordinal) + h.Length)..];
                                }

                                foreach (string header in hdr) {
                                    name += "\t";
                                    if (usedHdr.Count <= 0 || usedHdr[0] != header) continue;
                                    if (cols != null && !cols.StartsWith(header + ": "))
                                        throw new Exception();
                                    cols = cols?.Substring(header.Length + 1);
                                    string? elem;
                                    if (usedHdr.Count > 1) {
                                        int pos = cols.IndexOf("; " + usedHdr[1] + ": ", StringComparison.Ordinal);
                                        elem = cols[..pos];
                                        cols = cols[(pos + 2)..];
                                    } else {
                                        elem = cols;
            ...
            if (hdr == null) return new ListContent("ListView", -1, null, values.ToArray());
            {
